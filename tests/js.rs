...
fn test_string() {
    ...
    let v = js_helpers::js!(String::from("world")).unwrap();
...
fn test_array() {
    ...
    assert_ne!(v_6.get(2), js_helpers::js!([]).unwrap());
    assert_ne!(v_6.get(4), js_helpers::js!({}).unwrap());
...
fn test_array_access() {
    ...
    assert_eq!(js_helpers::js!(v[0]).unwrap().as_f64().unwrap(), 1.0);
    assert_eq!(js_helpers::js!(v[1]).unwrap().as_f64().unwrap(), 5.0);
    assert_eq!(js_helpers::js!(v[2][0]).unwrap().as_f64().unwrap(), 4.0);
    assert_eq!(js_helpers::js!(v[3][0][0]).unwrap().as_f64().unwrap(), 3.0);
    assert_eq!(js_helpers::js!(v[3][0][1]).unwrap().as_f64().unwrap(), 6.0);
    assert_eq!(js_helpers::js!(v[3][0][2]).unwrap(), wasm_bindgen::JsValue::UNDEFINED);
    assert_eq!(js_helpers::js!(v[3][1]).unwrap(), wasm_bindgen::JsValue::UNDEFINED);
    assert_eq!(js_helpers::js!(v[4]).unwrap(), wasm_bindgen::JsValue::UNDEFINED);
    match js_helpers::js!(v[4][17]).unwrap_err() {
        js_helpers::JsMacroError::Lookup { object: _, index } => assert_eq!(index, 17),
        x => panic!("{x:?}"),
    }
    assert_eq!(js_helpers::js!(v[v[3][0][0]][0][1]).unwrap().as_f64().unwrap(), 6.0);
    assert_eq!(js_helpers::js!(v[v[3][0][0]]["0"][1]).unwrap().as_f64().unwrap(), 6.0);
    assert_eq!(js_helpers::js!(v[v["3"][0][0]]["0"][1]).unwrap().as_f64().unwrap(), 6.0);
    assert_eq!(js_helpers::js!(v[v["3"]["0"]["0"]]["0"]["1"]).unwrap().as_f64().unwrap(), 6.0);
    assert_eq!(js_helpers::js!((v[v["3"]["0"]["0"]]["0"]["1"])).unwrap().as_f64().unwrap(), 6.0);
...
fn test_object_access() {
    ...
    assert_eq!(js_helpers::js!(v.hello).unwrap().as_f64().unwrap(), 45.0);
    assert_eq!(js_helpers::js!(v["hello"]).unwrap().as_f64().unwrap(), 45.0);
    assert_eq!(js_helpers::js!(v[("hello")]).unwrap().as_f64().unwrap(), 45.0);
    assert_eq!(js_helpers::js!(v.world.more).unwrap().as_f64().unwrap(), 22.0);
    assert_eq!(js_helpers::js!(v["world"].more).unwrap().as_f64().unwrap(), 22.0);
    assert_eq!(js_helpers::js!(v["world"]["more"]).unwrap().as_f64().unwrap(), 22.0);
    assert_eq!(js_helpers::js!(v.foo).unwrap(), wasm_bindgen::JsValue::UNDEFINED);
    match js_helpers::js!(v.foo.bar).unwrap_err() {
        js_helpers::JsMacroError::Lookup { object: _, index } => assert_eq!(index, "bar"),
        x => panic!("{x:?}"),
    }
    assert_eq!(js_helpers::js!(v.foo?.bar).unwrap(), wasm_bindgen::JsValue::UNDEFINED);
    match js_helpers::js!(v.foo?.bar.baz).unwrap_err() {
        js_helpers::JsMacroError::Lookup { object: _, index } => assert_eq!(index, "baz"),
        x => panic!("{x:?}"),
    }
    assert_eq!(js_helpers::js!(v.foo?.bar?.baz).unwrap(), wasm_bindgen::JsValue::UNDEFINED);
}
...
fn test_function_calls() {
    ...
    assert_eq!(js_helpers::js!(add(5, 4)).unwrap().as_f64().unwrap(), 9.0);
    assert_eq!(js_helpers::js!(v.add(5, 4)).unwrap().as_f64().unwrap(), 9.0);
    assert_eq!(js_helpers::js!(v["add"](5, 4)).unwrap().as_f64().unwrap(), 9.0);
    assert_eq!(js_helpers::js!(v.sub(5, 4)).unwrap().as_f64().unwrap(), 1.0);
    assert_eq!(js_helpers::js!(v.more.stuff.a(1, 6)).unwrap().as_f64().unwrap(), 7.0);
    assert_eq!(js_helpers::js!(v["more"].stuff.a(1, 6)).unwrap().as_f64().unwrap(), 7.0);
    assert_eq!(js_helpers::js!(v["more"]["stuff"].a(1, 6)).unwrap().as_f64().unwrap(), 7.0);
    assert_eq!(js_helpers::js!(v.stuff(2, 3).x).unwrap().as_f64().unwrap(), 2.0);
    assert_eq!(js_helpers::js!(v.stuff(2, 3).y.z).unwrap().as_f64().unwrap(), 3.0);
    assert_eq!(js_helpers::js!(v.deeper(6, 7)(2)()).unwrap().as_f64().unwrap(), 44.0);
    assert_eq!(js_helpers::js!((v.deeper(6, 7)(2))()).unwrap().as_f64().unwrap(), 44.0);
    assert_eq!(js_helpers::js!(((v.deeper(6, 7))(2))()).unwrap().as_f64().unwrap(), 44.0);
    assert_eq!(js_helpers::js!((((v.deeper)(6, 7))(2))()).unwrap().as_f64().unwrap(), 44.0);
    assert_eq!(js_helpers::js!(((((v).deeper)(6, 7))(2))()).unwrap().as_f64().unwrap(), 44.0);
    ...
    assert_eq!(js_helpers::js!(x[0]).unwrap().as_f64().unwrap(), 44.0);

    let x = js_helpers::js!({ zz: v.deeper(6, 7)(2)(), "yy": 12, "x x": 7 }).unwrap();
    assert_eq!(js_helpers::js!(x.zz).unwrap().as_f64().unwrap(), 44.0);
    assert_eq!(js_helpers::js!(x["zz"]).unwrap().as_f64().unwrap(), 44.0);
    assert_eq!(js_helpers::js!(x.yy).unwrap().as_f64().unwrap(), 12.0);
    assert_eq!(js_helpers::js!(x["yy"]).unwrap().as_f64().unwrap(), 12.0);
    assert_eq!(js_helpers::js!(x["x x"]).unwrap().as_f64().unwrap(), 7.0);
}

#[wasm_bindgen_test]
fn test_complex_root() {
    assert_eq!(js_helpers::js!([5,true,null]["0"]).unwrap().as_f64().unwrap(), 5.0);
    assert_eq!(js_helpers::js!([5,true,null]["1"]).unwrap().as_bool().unwrap(), true);
    assert_eq!(js_helpers::js!([5,true,null]["2"]).unwrap().is_null(), true);

    assert_eq!(js_helpers::js!({ test: 5, more: false, again: null }.test).unwrap().as_f64().unwrap(), 5.0);
    assert_eq!(js_helpers::js!({ test: 5, more: false, again: null }["test"]).unwrap().as_f64().unwrap(), 5.0);
    assert_eq!(js_helpers::js!({ test: 5, more: false, again: null }.more).unwrap().as_bool().unwrap(), false);
    assert_eq!(js_helpers::js!({ test: 5, more: false, again: null }["more"]).unwrap().as_bool().unwrap(), false);
    assert_eq!(js_helpers::js!({ test: 5, more: false, again: null }.again).unwrap().is_null(), true);
    assert_eq!(js_helpers::js!({ test: 5, more: false, again: null }["again"]).unwrap().is_null(), true);
}

#[wasm_bindgen_test]
fn test_special_ident() {
    assert_eq!(js_helpers::js!(null?.foo).unwrap().is_undefined(), true);
    assert_eq!(js_helpers::js!(undefined?.foo).unwrap().is_undefined(), true);
...
fn test_assignment() {
    ...
    assert_eq!(js_helpers::js!(x.foo).unwrap().as_f64().unwrap(), 45.0);
    assert_eq!(js_helpers::js!(x.foo = 20).unwrap().as_f64().unwrap(), 20.0);
    assert_eq!(js_helpers::js!(x.foo).unwrap().as_f64().unwrap(), 20.0);
    assert_eq!(js_helpers::js!(x["foo"] = 42).unwrap().as_f64().unwrap(), 42.0);
    assert_eq!(js_helpers::js!(x.foo).unwrap().as_f64().unwrap(), 42.0);

    assert_eq!(js_helpers::js!(x.baz.mer).unwrap().as_f64().unwrap(), 2.0);
    assert_eq!(js_helpers::js!(x.baz.mer = 7).unwrap().as_f64().unwrap(), 7.0);
    assert_eq!(js_helpers::js!(x.baz.mer).unwrap().as_f64().unwrap(), 7.0);
    assert_eq!(js_helpers::js!(x.baz["mer"] = -2).unwrap().as_f64().unwrap(), -2.0);
    assert_eq!(js_helpers::js!(x.baz.mer).unwrap().as_f64().unwrap(), -2.0);

    assert_eq!(js_helpers::js!(x.baz.der.length).unwrap().as_f64().unwrap(), 3.0);
    assert_eq!(js_helpers::js!(x.baz.der[0]).unwrap().as_f64().unwrap(), 3.0);
    assert_eq!(js_helpers::js!(x.baz.der[1]).unwrap().as_f64().unwrap(), 4.0);
    assert_eq!(js_helpers::js!(x.baz.der[2]).unwrap().as_f64().unwrap(), 2.0);
    assert_eq!(js_helpers::js!(x.baz.der[1] = 35).unwrap().as_f64().unwrap(), 35.0);
    assert_eq!(js_helpers::js!(x.baz.der.length).unwrap().as_f64().unwrap(), 3.0);
    assert_eq!(js_helpers::js!(x.baz.der[0]).unwrap().as_f64().unwrap(), 3.0);
    assert_eq!(js_helpers::js!(x.baz.der[1]).unwrap().as_f64().unwrap(), 35.0);
    assert_eq!(js_helpers::js!(x.baz.der[2]).unwrap().as_f64().unwrap(), 2.0);
    assert_eq!(js_helpers::js!(x.baz.der["4"] = 22).unwrap().as_f64().unwrap(), 22.0);
    assert_eq!(js_helpers::js!(x.baz.der.length).unwrap().as_f64().unwrap(), 5.0);
    assert_eq!(js_helpers::js!(x.baz.der[0]).unwrap().as_f64().unwrap(), 3.0);
    assert_eq!(js_helpers::js!(x.baz.der[1]).unwrap().as_f64().unwrap(), 35.0);
    assert_eq!(js_helpers::js!(x.baz.der[2]).unwrap().as_f64().unwrap(), 2.0);
